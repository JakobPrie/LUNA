...
class Audio_Input:
    def __init__(self, serverconnection, local_storage):
        ...
        self.userlist = []
        self.detector = None
        self.stt = Speech_to_Text(self.local_storage['LUNA_Modules_defined_Vocabulary'])
    ...
    def start_hotword_detection(self, sensitivity=0.5, audio_gain=1):
        self.stopped = False
        self.local_storage['LUNA_Hotword_detected'] = {}
        self.local_storage['LUNA_recognized_text'] = ''
...
###############################################################################################################################################################################
    ...
    def audioRecorderCallback(self, fname):
        ...
        self.local_storage['LUNA_recognized_text'] = text
        os.remove(fname)
    ...
    def listen(self):
        ...
        while self.local_storage['LUNA_recognized_text'] == '':
            time.sleep(0.01)
        self.detector.stopped = True
        text = self.local_storage['LUNA_recognized_text']
        self.local_storage['LUNA_recognized_text'] = ''
        print('\n- {}\n'.format(text))
        return text
