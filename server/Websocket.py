...
def startWsServer(luna, port: int, allowNonSecure: bool, listenTimeout: int):
    global EVENT_LOOP
    ...
    async def newConection(websocket: websockets.WebSocketServerProtocol, path):
        luna.Log.write('INFO', 'Neue WebSocket-Verbindung mit {}'.format(str(websocket.remote_address)), show=True)
            ...
            try:
                ...
                if action == 'listen':
                    ...
                    if user in AWAITING_USERS:
                        if not user in luna.local_storage['rooms'][room]['users']:
                            for myRoom in luna.local_storage['rooms']:
                                if user in luna.local_storage['rooms'][myRoom]['users']:
                                    luna.local_storage['rooms'][myRoom]['users'].remove(user)
                            luna.local_storage['rooms'][room]['users'].append(user)
                            luna.local_storage['users'][user]['room'] = room

                        AWAITING_USERS.remove(user)
                        QUEUED_MESSAGES[user] = msg
                    elif explicit:
                        if not user in luna.Users.userlist:
                            luna.Users.userlist.append(user)
                            if (' ' in user):
                                userdata = {
                                    'name': user,
                            ...
                            luna.Users.userdict[user] = userdata
                            luna.Modules.user_modules[user] = []
                        ...
                        if not luna.route_query_modules(user, text = msg, direct = True, origin_room = room, must_be_secure = room in SECURE_ROOMS):
                    ...
                    if 'output' in message and message['output'] is not None:
                        ...
                        output = 'auto'
                    luna.route_say('§nonexistent_websocket', msg, None, user, output)
                elif action == 'create_room':
                    room = message['room']
                    secure = message['secure']
                    if not room in luna.room_list and (allowNonSecure or secure):
                        if (secure):
                            SECURE_ROOMS.append(room)
                        luna.room_list.append(room)
                        luna.local_storage['rooms'][room] = {
                            'name': room,
                            'users':[]
                        }
    ...
    def runThread():
        luna.Log.write('INFO', 'WebSocket gestartet', show=True)
