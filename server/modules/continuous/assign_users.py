...
def start(luna, profile):
    profile['LUNA_voice_recognized_users'] = {}
    profile['LUNA_cam_recognized_users'] = {}

def assign(user, room, luna, profile):
    ...
    luna.rooms[room].users.append(user)
...
def run(luna, profile):
    ...
    for room in luna.rooms.copy().values():
        ...
        if not user == '':
            profile['LUNA_voice_recognized_users'][room.name] = user
            room.room_guessed_user = ''

    for room, users in profile['LUNA_cam_recognized_users'].items():
    ...
    for room, user in profile['LUNA_voice_recognized_users'].copy().items():
	...
	# Am Ende vom ganzen Text teilen wir den Räumen, die angefragt haben, dann auch noch mit,
        ...
        if not profile['rooms'][room]['users'] == []:
            if user in profile['rooms'][room]['users'] and not user == 'Unknown':
                luna.rooms[room].server_guessed_user = user
            else:
                luna.rooms[room].server_guessed_user = profile['rooms'][room]['users'][-1]
        else:
            luna.rooms[room].server_guessed_user = 'Unknown'
        del profile['LUNA_voice_recognized_users'][room]
