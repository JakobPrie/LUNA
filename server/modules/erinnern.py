    ...
    if 'lösch' in text:
        ...
        for item in erinnerungen:
            if item['Zeit'] is time:
                founded.append(item)
        if len(founded) is 0:
            luna.say('Ich habe leider keine Erinnerung zu dieser Zeit gefunden. Ich werde mal nach deinen Einträgen gucken.')
            sleep(2)
            entries = []
            for item in errinerungen:
                if item['Benutzer'] is luna.user:
                    entries.append(item)
                luna.say('Ich habe folgendes herausgefunden: du hast {} Erinnerungseinträge. Soll ich sie dir Vorlesen und du sagst, wenn es das richtige ist?'.format(str(len(entries))))
                answer = luna.listen()
                if 'ja' in answer:
                    for item in entries:
                        luna.say('Okay, ich werde dir einfach die Einträge vorlesen und nach jedem Eintrag sagst du entweder ja oder nein.')
                        hit = False
                        for item in entries:
                            if not hit:
                                luna.say(item['Text']  + ' Ist das richtig?')
                                resposne = luna.listen()
                                if 'ja' in response:
                                    founded = True
