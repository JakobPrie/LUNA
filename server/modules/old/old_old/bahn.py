...
def handle(text, luna, profile):
    ...
    try:
        ...
        if start is 'None':
            luna.say(speechVariation("Ich weiß noch gar nicht, wo [wir hier gerade sind|es losgehen soll]."))
            luna.say(speechVariation("An welche[m Bahnhof|r Station] [willst|möchtest] du [|denn] [|gerne] [ab|los]fahren?"))
            start = luna.listen().strip()
        startB = TrainStation(start)
        if "nach " in text:
            luna.say(speechVariation(["Mal sehen, was so auf den Schienen los ist.", "Einen Augenblick bitte."]))
        ...
        if journey:
            ...
            if batchMatch("[ja|ja gerne|gern]", instr):
                ...
                luna.say("[Okay|Kein Problem|keine Ursache]")
