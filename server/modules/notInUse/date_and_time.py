...
def handle(text, luna, profile):
    ...
    if 'uhr' in text or 'spät' in text or 'uhrzeit' in text:
        ...
        if 'guten morgen' in text:
            luna.say('Bist du etwa gerade erst aufgewacht?')
            antwort_eins = luna.listen()
            ...
            if 'ja' in antwort_eins or 'warum nicht' in text or 'ich war müde' in antwort_eins or 'ich war halt müde' in antwort_eins or 'jetzt bin ich ja wach' in antwort_eins or 'jetzt bin ich jedenfalls wach' in antwort_eins:
                luna.say('Du solltest wirklich nachts schlafen, sonst bringst du noch deinen Schlafrhytmus durcheinander!')
            else:
                luna.say('In Ordnung.')
        elif 'guten abend' in text:
            luna.say('Guten Abend, {}. Hast du heute noch was vor?'.format(luna.user))
            antwort_eins = luna.listen()
            antwort_eins = antwort_eins.lower()
            if 'TIMEOUT_OR_INVALID' in antwort_eins:
                luna.say('Ich konnte deine Pläne leider nicht verstehen.')
                ...
                luna.say('Dann genieße den freien Abend!')
            else:
                luna.say('Das klingt toll! Ich wünsche dir viel Spaß dabei!')
            ...
            antwort_eins = antwort_eins.lower()
            if 'TIMEOUT_OR_INVALID' in antwort_eins:
                luna.say('Ich fürchte, ich habe dich nicht ganz verstanden. Soll ich dich morgen früh wecken?')
                antwort_zwei = luna.listen()
                antwort_zwei = luna_zwei.lower()
                if 'ja' in antwort_zwei or 'weck mich' in antwort_zwei or 'wecke mich' in antwort_zwei or 'ich mÃ¶chte um' in antwort_zwei or 'ich will um' in antwort_zwei or 'bitte um' in antwort_zwei:
                    luna.start_module(text=antwort_zwei)
                elif 'nein' in antwort_zwei or 'nicht' in antwort_zwei:
                    luna.say('In Ordnung. Schlaf gut, {}'.format(luna.user))
                else:
                    luna.say('Ich konnte dich leider nicht verstehen. Du bist wohl schon zu müde.')
                ...
                luna.start_module(text=antwort_eins)
        ...
        if 'guten morgen' in text or 'guten tag' in text:
            ...
            if 'nein' in antwort_eins or 'ich hatte besseres zu tun' in antwort_eins or 'ich bin nicht dazu gekommen' in antwort_eins or 'ich bin noch nicht dazu gekommen' in antwort_eins or 'ich bin zu beschäftigt' in antwort_eins or 'schlaf ist für die schwachen' in antwort_eins:
                luna.say('Du solltest wirklich ins Bett gehen! 23 Stunden ohne Schlaf sind nicht gut für deinen Körper!')
            else:
                luna.say('Ich konnte dich leider nicht verstehen. Du bist wohl noch zu müde.')
    elif uhrzeit >= 0 and uhrzeit <= 4:
        if 'guten morgen' in text or 'guten tag' in text:
            luna.say('Schlaf ruhig weiter, es ist noch mitten in der Nacht!')
            antwort_eins = luna.listen()
            antwort_eins = antwort_eins.lower()
            ...
            if 'ich bin wach' in antwort or 'etwas vor' in antwort or 'muss los' in antwort or 'müssen los' in antwort or 'jetzt stehe ich auf' in antwort or 'egal' in antwort:
                luna.say('Alles klar! Wie wäre es mit einem Focusgetränk?')
                antwort_zwei = luna.listen()
                antwort_zwei = antwort_zwei.lower()
                if 'ja' in antwort_zwei or 'gerne' in antwort_zwei or 'wäre jetzt gut' in antwort_zwei or 'warum nicht' in antwort_zwei:
                    luna.say('Wenn endlich mal jemand ein Emporgymaschienen Modul programmieren würde, würde ich dir jetzt sehr gerne einen machen.') #Das ist gut, er wird dich aufwecken.
                elif 'nein' in antwort_zwei or 'nicht' in antwort_zwei:
                    luna.say('In Ordnung')
                else:
                    luna.say('Ich konnte dich leider nicht verstehen. Du bist wohl schon zu müde.')
        ...
        if 'guten morgen' in text or 'guten tag' in text:
            ...
            luna.say('Willst du etwa schon schlafen gehen, {}?'.format(luna.user))
            antwort_zwei = luna.listen()
            a_z = antwort_zwei.lower()
            if 'ja' in a_z or 'warum nicht' in a_z or 'klar' in a_z:
                luna.say('Dann schlaf gut, aber vergiss nicht, auch wieder aufzuwachen!')
            elif 'nein' in a_z or 'noch nicht' in a_z or 'natürlich nicht' in a_z:
                luna.say('Alles klar, dann wünsche ich dir noch einen schönen Abend!')
            else:
                luna.say('Ich konnte dich leider nicht verstehen. Du bist wohl doch schon zu müde.')
        elif 'guten abend' in text:
            luna.say('Ob es schon Abend ist, liegt wohl im Blickwinkel des Betrachters. Ich würde sagen, es ist gerade eher Nachmittag.')
        ...
        if 'guten morgen' in text or 'guten tag' in text:
            ...
            if 'ja' in a_z or 'warum nicht' in a_z or 'klar' in a_z:
                ...
                luna.say('Alles klar, dann wünsche ich dir noch einen schönen Tag!')
            else:
                luna.say('Ich konnte dich leider nicht verstehen, {}.'.format(luna.user))
        elif 'guten abend' in text:
            luna.say('Ob es schon Abend ist, liegt wohl im Blickwinkel des Betrachters. In Asien ist es jetzt in der Tat Abend.')
    else:
        luna.say('Guten Tag, {}'.format(luna.user))
